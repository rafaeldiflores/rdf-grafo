...
app.get("/authorize", async (c) => {
	...
	return renderApprovalDialog(c.req.raw, {
		...
		csrfToken,
		server: {
			description: "Postulador de Rafael Díaz Flores: CVs y Tracker de postulaciones. Acceso privado.",
			logo: "https://avatars.githubusercontent.com/u/143029098?s=200&v=4",
			name: "Postulador RDF",
		},
		setCookie,
		state: { oauthReqInfo },
...
app.get("/callback", async (c) => {
	...
	const { login, id } = user.data;

	// SOLO el dueño del vault: se compara el id numérico (inmutable), no el login
	// (que se puede cambiar o reutilizar). A cualquier otra cuenta no se le emite token.
	if (String(id) !== String(c.env.ALLOWED_GITHUB_ID)) {
		console.warn("Acceso denegado a una cuenta de GitHub no autorizada");
		return c.text("Acceso denegado: este servidor es privado.", 403);
	}

	// Return back to the MCP client a new token. El token de GitHub NO se guarda:
	// el servidor no lo necesita (usa su propio token limitado a rdf-vault).
	const { redirectTo } = await c.env.OAUTH_PROVIDER.completeAuthorization({
		metadata: { label: login },
		props: { githubId: id, login } as Props,
